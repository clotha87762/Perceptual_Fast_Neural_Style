...
class transform(object):
    ...
    def generator_unet1(self , _input , reuse = False ): # origin unet
        with tf.variable_scope('generator'):
            ...
            with slim.arg_scope([instance_norm],decay = 0.9, epsilon = 1e-5):
    ...
    def generator_resnet(self , _input, reuse = False): # resnet
        with tf.variable_scope('generator'):
            ...
            def residual(_input , dim,  kernel =[3,3] ,stride = 1  ,scope ):
                x = slim.conv2d(_input, dim, kernel, stride , scope = scope)
                x = slim.conv2d( relu(x) , dim, kernel, stride , scope = scope)
                res = x + _input
                return res
            ...
            with slim.arg_scope([instance_norm],decay = 0.9, epsilon = 1e-5):
                ...
                c2 = relu( instance_norm(c1,is_train = self.is_training ,name ='g_e2_bn' ) )
                ...
                d0 = relu( instance_norm(d0,is_train = self.is_training , scope = 'g_d0_bn'))
                d1 = deconv2d_resize(d0 , self.gfdim, kernel=3, stride=(2,2), name = 'g_d1_conv')
                d1 = relu( instance_norm(d1,is_train = self.is_training , scope = 'g_d1_bn'))
                
                d2 = slim.conv2d(d1 , self.out_dim , [9,9] , 1 , scope = 'g_d2_conv')
                d2 =  instance_norm(d2,is_train = self.is_training , scope = 'g_d2_bn')
